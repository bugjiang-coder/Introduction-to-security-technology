...
static  void PC_2(const int input[56], int output[48], int table[48])//PC_2  左移 的子密钥（拼起来） 置换选择2 56-48 
{
	int i;
	for (i = 0; i<48; i++)
	{
...
static void F_func(int input[32], int output[32], int subkey[48])//完成DES算法轮变换  
{
	...
	int temp[48] = { 0 };
	int temp_1[32] = { 0 };
	E(input, temp, E_Table); //temp输出 
	Xor(temp, subkey, len); //异或temp子密钥 
	S(temp, temp_1, S_Box);  // 48位异或后的temp进入s盒变换变成32位temp_1 
	P(temp_1, output, P_Table); //temp_1再进行p置换，output为输出 
};
 
static void  subKey_fun(const int input[64], int Subkey[4][48])//子密钥生成  
{
	int loop = 1, loop_2 = 2;
	int i, j;
	int c[28], d[28];
...
static void  DES_Efun(char input[8], char key_in[8], int output[64])
{
	...
	for (int j = 1; j<4; j++)//前3轮的操作  
	{
		...
		F_func(r[j - 1], r[j], subkeys[j - 1]);   //右进行f变换 输出为r[j] 
		Xor(r[j], l[j - 1], 32);  //r[j]进行异或变换
		...
		printf("  "); 
		for(x=0;x<32;x++)
			printf("%d",r[j][x]);  //此时j=3 
	}
	
	int t = 0;
	for (t = 0; t<32; t++)//最后一轮的操作  
	{
		r[4][t] = r[3][t];              //l[i]=r[i-1]
	}
	F_func(r[3], l[4], subkeys[3]);     //r[i]=f(l[i-1],r[i-1],k[i])
	Xor(l[4], l[3], 32);
